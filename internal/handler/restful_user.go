...
func (w *restfulHandler) loginuser(c *gin.Context) {
	type Request struct {
	...
	if param.UserName == "admin" && param.PassWord == "pdSec666" {
		token = "adminPdSec666"
	} else {
		ReturnError(c, errOauth, nil)
		return
	}

	u, err := w.db.GetUserByToken(token, false)
	if err != nil {
